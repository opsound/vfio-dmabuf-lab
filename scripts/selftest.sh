...
check "dry-run kernel" "v6:nvgrace-v6
v6:dmabuf
v6:reset-lockdep" "$("${runner}" --dry-run v6)"

# A bare test name runs its default kernel's entry only.
check "dry-run test" "v6:reset-lockdep" "$("${runner}" --dry-run reset-lockdep)"
...
expect_fail "dry-run empty k:t part" "${runner}" --dry-run 'v6:'
...
expect_fail "extra selection" "${runner}" v6 v5
